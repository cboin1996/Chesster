...
Module taken from zeta36's github.
https://https://github.com/Zeta36/chess-alpha-zero
"""
import os
import re
...
from deep_learning.lib.data_ops import write_game_data_to_file, find_pgn_files
...
class SupervisedLearningWorker:
    ...
    Attributes:
        Config config: config for this worker
        list((str,list(float)) buffer: buffer containing the data to use for training -
        ...
        with ProcessPoolExecutor(max_workers=7) as executor:
            ...
            for res in as_completed([executor.submit(get_buffer, self.config, game) for game in games]): #poisoned reference (memleak)
                ...
                logger.debug(f"game {self.idx:4} time={(end_time - start_time):.3f}s "
                             f"halfmoves={env.num_halfturns:3} {env.victor:12}"
                             f"{' by resign ' if env.isResigned else '           '}"
                             f"{env.get_fen().split(' ')[0]}")
                start_time = end_time
...
def get_buffer(config, game) -> (Chess, list):
    """
    ...
    """
    env = Chess().reinit()
    white = Player(config, dummy=True)
    black = Player(config, dummy=True)
    ...
    k = 0
    while not env.over() and k < len(actions):
        if env.whites_turn():
            action = white.sl_action(env.get_fen(), actions[k], weight=white_weight) #ignore=True
        else:
            action = black.sl_action(env.get_fen(), actions[k], weight=black_weight) #ignore=True
        env.make_move(action, False)
        k += 1

    if not env.board.is_game_over() and result != '1/2-1/2':
        env.isResigned = True
    if result == '1-0':
        env.victor = Victor.white
        black_win = -1
    elif result == '0-1':
        env.victor = Victor.black
        black_win = 1
    else:
        env.victor = Victor.draw
        black_win = 0
